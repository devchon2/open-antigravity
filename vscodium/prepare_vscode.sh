...
SRC_DIR="$SCRIPT_DIR/open-antigravity-src"
...
INJECT_SCRIPT="$VSC_DIR/inject_agent.sh"
cat > "$INJECT_SCRIPT" << 'INNEREOF'
#!/bin/bash
# Injects Open-Antigravity workbench source into the cloned vscode tree
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
AG_SRC="$SCRIPT_DIR/../open-antigravity-src/vs/workbench/contrib/antigravity"
...
if ! grep -q "antigravity" "$CONTRIB_FILE" 2>/dev/null; then
  echo "import '../contrib/open-antigravity/antigravity.contribution.js';" >> "$CONTRIB_FILE"
...
if [ -f "$GET_REPO" ]; then
  if ! grep -q "inject_agent" "$GET_REPO" 2>/dev/null; then
    ...
    echo 'bash "$SCRIPT_DIR/inject_agent.sh"' >> "$GET_REPO"
