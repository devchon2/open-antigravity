...
export interface ArtifactComment {
  id: string; author: 'user' | 'agent'; content: string; timestamp: string;
}

export interface Artifact {
  id: string; agentId: string; type: ArtifactType; title: string; content: string;
  status: ArtifactStatus; comments: ArtifactComment[]; parentId?: string;
...
export class AntigravityArtifactStore {
  ...
  async save(): Promise<void> {
    try {
      ...
      const dir = join(process.cwd(), '.antigravity');
  ...
  async load(): Promise<void> {
    try {
      ...
      for (const a of d) this.artifacts.set(a.id, a);
    } catch {}
  }
}
